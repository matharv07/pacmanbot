...
_pac.AUTO_MODE = True                     # AI pacman for training
...
class Env:
    """One headless game instance."""

    def __init__(self, env_id: int = 0, num_ghosts: int = _DEFAULT_GHOSTS,
                 grid_rows: int = _DEFAULT_ROWS, grid_cols: int = _DEFAULT_COLS,
                 n_power: int = _DEFAULT_POWER):
        self.env_id     = env_id
    ...
    def reset(self):
        ...
        self.shaper.reset()
        self.recent_nom = {
            i: np.zeros((self.grid_rows, self.grid_cols), dtype=np.float32)
            for i in range(self.num_ghosts)
        }
        self._cached_ht = {}   # populated on first step()
    ...
    def step(self, action_dict: dict):
        ...
        Parameters
        ----------
        action_dict : gid → (indices: list[(r,c)], scores: ndarray (H, W))

        Returns  (obs, rewards: dict, done: bool, info: dict)
        """
        alive = [gid for gid, g in self.ghosts.items() if not g.dead]

        # apply RL waypoint nominations to CBBA
        for gid in alive:
            g = self.ghosts[gid]
        ...
        for _ in range(DECISION_INTERVAL):
            ...
            for gid, ghost in list(self.ghosts.items()):
                new_cells, stale_refresh = ghost.update((self.player.row, self.player.col),
                                                        powered, self.ghosts)
                if gid in rewards:
            ...
            if not self.player.dead:
                for gid, ghost in list(self.ghosts.items()):
                    ...
                    swap = (ghost.row == self.player.prev_row
                            and ghost.col == self.player.prev_col
                            and self.player.row == ghost.prev_row
                            and self.player.col == ghost.prev_col)
                    if same or swap:
        ...
        # shaping (at the decision boundary)
